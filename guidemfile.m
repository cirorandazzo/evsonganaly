...
%GUIDEMFILE Support function for GUIDE. Support GUI M-file.

%   $Revision: 1.1.6.29 $ $Date: 2010/09/24 14:34:32 $
...
fid = fopen(filename);
if fid == -1
    error(['MATLAB:', mfilename, ':FileNotFound'], '%s does not exist.', filename);
else
...
if fid == -1
    error(['MATLAB:', mfilename, ':NoFile'], '%s does not exist or is not writable.',filename);
else
    ...
    for cb = 1:length(callbacks)
        if isAutoCallback(handle(all_h(i)),callbacks{cb})
            ...
            if strcmp(n, funcname)
                ...
                setAutoGeneratedCallbackString(all_h(i),callbacks{cb},formAutoGeneratedCallbackString(all_h(i),callbacks{cb},funcname));
                ...
                if isempty(strmatch(subfcnLookup, existingSubfuncs, 'exact'))
    ...
    if ~isempty(mfunc) && ~isempty(subfunc)
        ...
        if ~isempty(where) && where(1)==1 
            ...
            if lineno
                ...
                message = ['Cannot find function %s in GUI M-file:\n%s\n\n'...
                           'To correct this problem and make the %s work, please make sure '...
                           'the function name %s is correctly specified and the function exists in the GUI M-file.'...
                           'You may also ask GUIDE to auto-generate a callback for you.'];
                warndlg(sprintf(message, subfunc, mfunc, whichCb,subfunc), 'GUIDE');
            end
        end
    end        
end
...
if ~isempty(childlist)
    ...
    if ~feature('HGUsingMATLABClasses') && ~isempty(find(isprop(handlelist(titles),'TitleHandle'), 1))
        oldWarning = warning('off','MATLAB:Uipanel:HiddenImplementation');
        thandles= get(handlelist(titles),'TitleHandle');
        warning(oldWarning.state, 'MATLAB:Uipanel:HiddenImplementation');
...
function children = getToolbarToolInFigure(fig)
    ...
    if ishghandle(fig)
        ...
        children = [flipudJC(toolbars); flipudJC(children)];
    end
...
if ~ischar(value)
   try
       ...
       if isbuttongroup(h) && isequal(callback,'SelectionChangeFcn')
...
% *************************************************************************
% handle new HG2 type names
% *************************************************************************
function result = isbuttongroup(obj)

result = isa(handle(obj),'uitools.uibuttongroup') || isa(handle(obj),'matlab.ui.container.ButtonGroup');
